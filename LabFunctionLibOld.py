...
class ts_data(df.Dataframe):
    ...
    def calc_outlet_flowrate(self):
        """Calculate total outlet flowrate in moles/s"""
        self.data['outlet_flowrate'] = self.data['mass_flow_argon_tracer']/(self.data['Argon_MS']/100)/24.13/60
...
##            'mass_flow_argon_tracer']+self.data['mass_flow_argon_tracer']*self.data['N2_GC']/self.data['Ar_GC'])/24.134/60
        gas_produced = self.data['outlet_flowrate']-(self.data['mass_flow_entrainment']+self.data[
            'mass_flow_argon_tracer']+self.data['mass_flow_feed_vessel_pressure']-0.3)/24.13/60
    ...
    def calc_carbon_out(self):
        """Calculates outlet carbon flow in gas in moles/s"""
        ...
        for k in c_content:
            carbon_out = self.data[k+'_MS_flowrate']*c_content[k]
    ...
    def calc_y_co(self):
        return np.sum(self.data['Carbon Monoxide_MS_flowrate'])/np.sum(self.data['carbon_in'])

    def calc_ratio_co_h2(self):
        return np.sum(self.data['Carbon Monoxide_MS_flowrate'])/np.sum(self.data['Hydrogen_MS_flowrate'])

    def calc_ratio_co_co2(self):
        return np.sum(self.data['Carbon Monoxide_MS_flowrate'])/np.sum(self.data['Carbon Dioxide_MS_flowrate'])

    def calc_ratio_co_ch4(self):
        return np.sum(self.data['Carbon Monoxide_MS_flowrate'])/np.sum(self.data['Methane_MS_flowrate'])
...
class Biomass:
    def __init__(self, CHN = None, moisture = None, enthalpy_formation = None):
    ...
    def calc_CHN_dry(self):
        self.CHN_dry={}
        self.CHN_dry['C']=self.CHN['C']/(100.-self.moisture)*100
        self.CHN_dry['H']=(self.CHN['H']-self.moisture*2.02/18.02)/(100.-self.moisture)*100
        self.CHN_dry['N']=self.CHN['N']/(100.-self.moisture)*100
...
class Stream:
    ...
    species['C3H6'] = {'C':3, 'H':8, 'name':'Propylene'}
...
##    d['GAS20121204g'] = ['2012-12-4 11:01:00', '2012-12-4 12:04:00', '12040814.csv' , 0.5229]
##    d['GAS20121204d'] = ['2012-12-5 20:31:00', '2012-12-5 21:27:00', '12050907.csv' , 0.53]
##    d['GAS20121204b'] = ['2012-12-10 15:09:00', '2012-12-10 16:05:00', '12101214.csv' , 0.522]
    d['test'] = ['2012-12-18 10:56:00', '2012-12-18 11:56:00', '12101214.csv' , 0.522]
...
##        data.get_ms_from_csv(d[k][2])
        data.interp_ga()
##        data.calc_outlet_flowrate()
##        data.calc_gas_produced_flowrate()
##        data.calc_product_flowrates()
##        data.calc_carbon_in(d[k][3])
...
##        print 'Average Feed Rate: ' + str(round(np.average(data['mass_flow_biomass_from_ktron']+0.03),2)) + ' lbs/hr'
##        print 'StD Feed Rate: ' + str(round(np.std(data['mass_flow_biomass_from_ktron']+0.03),2)) + ' lbs/hr'
...
##        print 'Y CO: ' + str(round(np.sum(data['Carbon Monoxide_MS_flowrate'])/np.sum(data['carbon_in']),3))
##        print 'CO/H2: ' + str(round(np.sum(data['Carbon Monoxide_MS_flowrate'])/np.sum(data['Hydrogen_MS_flowrate']),3))
##        print 'CO/CO2: ' + str(round(np.sum(data['Carbon Monoxide_MS_flowrate'])/np.sum(data['Carbon Dioxide_MS_flowrate']),3))
##        print 'CO/CH4: ' + str(round(np.sum(data['Carbon Monoxide_MS_flowrate'])/np.sum(data['Methane_MS_flowrate']),3)) +'\n'
##        plt.plot(data['timestamp'], data['mass_flow_biomass_from_ktron']+0.03)
